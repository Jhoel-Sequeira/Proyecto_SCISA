...
def login():
    ...
    if request.method == "POST":
        ...
        db1.execute("Update Usuario set Contra = :userhash WHERE Usuario =:user",
                    userhash=generate_password_hash("123"), user="Maria.Mendez")
    ...
    if request.method == "POST":
        ...
        if sele:
            if sele == 'todo':
                ...
                proyectos_user = db.execute(
                    "SELECT * FROM Proyectos WHERE Empleado = :user", user=session["user_id"])
                ...
                completado = db.execute(
                    "SELECT COUNT(Estado)From Proyectos WHERE Estado = :estado", estado="Completado")
                Incompleto = db.execute(
                    "SELECT COUNT(Estado)From Proyectos WHERE Estado = :estado", estado="Incompleto")
                Progreso = db.execute(
                    "SELECT COUNT(Estado)From Proyectos WHERE Estado = :estado", estado="Progreso")
                imagen = db.execute(
                    "SELECT Imagen From Reportes WHERE Id_reporte = :estado", estado=1)
                total = db.execute("SELECT COUNT(*) FROM Proyectos")
                cuentaComp = db.execute(
                    "SELECT count(*)*100 / :to AS calc From Proyectos WHERE Estado = :est", est="Completado", to=total[0]['COUNT(*)'])
                cuentaPro = db.execute(
                    "SELECT count(*)*100 / :to AS calc From Proyectos WHERE Estado = :est", est="Progreso", to=total[0]['COUNT(*)'])
                cuentaInc = db.execute(
                    "SELECT count(*)*100 / :to AS calc From Proyectos WHERE Estado = :est", est="Incompleto", to=total[0]['COUNT(*)'])
                return render_template('home.html', var1="", completo=cuentaComp[0]['calc'], progreso=cuentaPro[0]['calc'],
                                       ...
                                       comp=completado[0]["COUNT(Estado)"], inco=Incompleto[0][
                    "COUNT(Estado)"], prog=Progreso[0]["COUNT(Estado)"], img=imagen,
                    indice="todo")

            else:
                print("meses")
                proyectos_especificos = db.execute(
                    "SELECT * FROM Proyectos WHERE strftime('%m', FechaInicio) = :mes", mes=sele)
                proyectos_user = db.execute(
                    "SELECT * FROM Proyectos WHERE Empleado = :user ", user=session["user_id"])
                solicitudes = db.execute("SELECT * FROM Solicitudes")
                # contar los proyectos aprobados, incompletos y en progreso
                completado = db.execute(
                    "SELECT COUNT(Estado)From Proyectos WHERE Estado = :estado AND strftime('%m', FechaInicio)=:mes", estado="Completado", mes=sele)
                Incompleto = db.execute(
                    "SELECT COUNT(Estado)From Proyectos WHERE Estado = :estado AND strftime('%m', FechaInicio)=:mes", estado="Incompleto", mes=sele)
                Progreso = db.execute(
                    "SELECT COUNT(Estado)From Proyectos WHERE Estado = :estado AND strftime('%m', FechaInicio)=:mes", estado="Progreso", mes=sele)
                imagen = db.execute(
                    "SELECT Imagen From Reportes WHERE Id_reporte = :estado", estado=1)
                total = db.execute(
                    "SELECT COUNT(*) FROM Proyectos WHERE strftime('%m', FechaInicio) = :mes", mes=sele)
                cuentaComp = db.execute("SELECT count(*)*100 / :to AS calc From Proyectos WHERE Estado = :est AND strftime('%m', FechaInicio) = :mes",
                                        est="Completado", to=total[0]['COUNT(*)'], mes=sele)
                cuentaPro = db.execute("SELECT count(*)*100 / :to AS calc From Proyectos WHERE Estado = :est AND strftime('%m', FechaInicio) = :mes",
                                       est="Progreso", to=total[0]['COUNT(*)'], mes=sele)
                cuentaInc = db.execute("SELECT count(*)*100 / :to AS calc From Proyectos WHERE Estado = :est AND strftime('%m', FechaInicio) = :mes",
                                       est="Incompleto", to=total[0]['COUNT(*)'], mes=sele)
                indice = "mes"
                print(indice)
            ...
            return render_template('home.html', var1="", completo=cuentaComp[0]['calc'], progreso=cuentaPro[0]['calc'],
                                   ...
                                   soli=solicitudes, comp=completado[0]["COUNT(Estado)"], inco=Incompleto[
                0]["COUNT(Estado)"],
        ...
        cuentaPro = db.execute(
            "SELECT count(*)*100 / :to AS calc From Proyectos WHERE Estado = :est", est="Progreso", to=total[0]['COUNT(*)'])
        cuentaInc = db.execute("SELECT count(*)*100 / :to AS calc From Proyectos WHERE Estado = :est",
                               est="Incompleto", to=total[0]['COUNT(*)'])
        ...
        if sele:
            if sele == 'todo':
                ...
                cuentaInc = db.execute(
                    "SELECT count(*)*100 / :to AS calc From Proyectos WHERE Estado = :est", est="Incompleto", to=total[0]['COUNT(*)'])
                variable = db.execute(
                    "SELECT * FROM Proyectos WHERE Empleado = :user", user=info)
